"""
自己写分页展示，用offset和limit参数；
后又用自定义redis连接，实现存储未读消息数
"""
...
from .serializer_v3 import MsgListSerializer, MessageSerializer
...
from magic_chat.utils.redis import set_user_msg_count
...
class Message(APIView):
    ...
    def get(self, request: Request):
        ...
        if start_time:
            all_msg = ModelMessage.objects.filter(chat=user_chat,send_time__lt=start_time).all().order_by('-send_time')
            # 取切片[offset:offset+limit]
            msg = all_msg[offset:offset+limit]
        # 如果没有start_time参数，就是查看最新消息；order_by('-send_time')倒序；取出对应会话最近的10条消息
        else:
            all_msg = ModelMessage.objects.filter(chat=user_chat).all().order_by('-send_time')
            # 取切片[offset:offset+limit]
            msg = all_msg[offset:offset+limit]
            ...
            user_chat.save()
            # 使用自定义的redis连接，给自己用户的未读消息数清零；用到了reids.py里面自定义的函数
            redis_key = f"{request.user.user_id}_{str(user_chat.id)}_msg_count"
            set_user_msg_count(redis_key,0)

        msg_data = MessageSerializer(msg,many=True).data
        context = {
            "status": status.HTTP_200_OK,
            "msg": "获取该会话的消息成功",
            "all_page_num":math.ceil(len(all_msg) / limit),   # 共有多少页
            "data": msg_data,
        }
        return Response(context)
    ...
    def post(self, request: Request):
        ...
        # 使用自定义的redis连接，给对方用户的未读消息数加1；用到了reids.py里面自定义的函数
        redis_key = f"{to_user.user_id}_{str(chat.id)}_msg_count"
        set_user_msg_count(redis_key)
